...
### 2-5. 조건절 상수화 이슈 (로컬변수, 사용자정의함수), 테이블 변수 최적화 이슈
...
```
- Query에서 Index를 사용할 수 있는지 판단은 Compile 시점이나, 로컬변수의 값을 알 수 있는 시점은 Runtime 시점으로, Complie 시점에는 Unknown상태이다. Optimizer ㅅ
