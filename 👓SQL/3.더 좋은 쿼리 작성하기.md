...
1. Nested Loops Join 
	...
	2. ex) Customers 테이블에서 조회된 데이터를 가지고 BigOrders를 조회한다. Customers  테이블에서 200건의 데이터가 조회되면 BicOrders에 검색을 200번의 반복이 일어난다.
	3. 성능을 생각한다면,,
...
#### 11-2.조인 열 변형 이슈
- 사례 - 모델링 이슈
	- JOIN하는 열에 SARG를 위반하여 Index 사용을 못 함. ()
	- C 테이블의 칼럼들을 WHERE절 검색 칼럼으로 사용하여, Optimizer는 C 테이블을 외부 입력으로 해서 먼저 검색을 하고 검색된 결과 건수를 P테이블을 내부 입력으로 해서 같은 아이디를 가진 값을 조인으로 결합하는 동작을 반복 수행할 것.
```
C.Index varchar(300)
P.ID    int
