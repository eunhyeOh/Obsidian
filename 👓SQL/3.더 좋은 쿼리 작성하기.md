...
1. 결과에 부모 집합 요구, 자식은 선택 연산만 수행
	1. 부모:자식 = 1:N으로 BigOrders만큼 Customers 중복 데이터가 만들어지게 된다.
	2. 자식 테이블의 칼럼은 SELECT절에 없고 중복 데이터가 만들어지는 부모테이블 칼럼만 반환하여 중복 제거하는 DISTINCT를  불필요하게 사용했음.
	3. SELECT 결과 집합에 참여하지 않고 데이터 존재여부만 확인하는 용도
``` 
1:N 관계
